...
export function HistoryCard({
  ...
  const changedProducts = calculateChangedProducts(entry.fridge_state_before, entry.fridge_state_after);

  // Filtruj tylko faktycznie użyte składniki (te, które się zmieniły)
  const usedIngredients = entry.fridge_state_before.items.filter((item) => changedProducts.includes(item.product_id));
  ...
  return (
    <Card className="hover:shadow-md transition-shadow duration-200 border-l-4 border-l-primary">
      <CardHeader className="space-y-3">
        ...
        <UsedIngredientsPreview ingredients={usedIngredients} maxDisplay={3} />
      </CardHeader>
      ...
      <CardFooter>
        <Button onClick={handleCookAgain} variant="default" className="w-full sm:w-auto">
