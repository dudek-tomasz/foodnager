...
export default function IngredientsDeductionPreview({ ingredients }: IngredientsDeductionPreviewProps) {
  // Pokazuj tylko składniki dostępne (full lub partial)
  const availableIngredients = ingredients.filter((ing) => ing.availabilityStatus !== "none");

  if (availableIngredients.length === 0) {
    return <p className="text-sm text-gray-500 dark:text-gray-400">Brak składników do odjęcia</p>;
  }

  return (
    <div className="space-y-3">
      <p className="text-sm text-gray-600 dark:text-gray-400">Następujące składniki zostaną odjęte z Twojej lodówki:</p>

      <ul className="space-y-2 max-h-64 overflow-y-auto">
        {availableIngredients.map((ingredient, index) => {
          const deductQuantity =
            ingredient.availabilityStatus === "full" ? ingredient.requiredQuantity : ingredient.availableQuantity;
          ...
          return (
            ...
            >
              <span className="font-medium text-gray-900 dark:text-gray-100">{ingredient.product.name}</span>
              <span className="text-sm text-gray-600 dark:text-gray-400">
                - {formatQuantity(deductQuantity, ingredient.unit.name, ingredient.unit.abbreviation)}
              </span>
            </li>
          );
        })}
      ...
      {availableIngredients.some((ing) => ing.availabilityStatus === "partial") && (
        <div className="flex items-start gap-2 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-md">
          ...
          <p className="text-sm text-yellow-800 dark:text-yellow-200">
            Niektóre składniki są dostępne tylko częściowo. Przepis może wymagać dodatkowych produktów.
          </p>
        </div>
      )}
    </div>
