...
export default function SourceSelectionView({
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-gray-900 mb-8">Wybierz źródło przepisów</h1>

      {/* Fridge status banner */}
      <FridgeStatusBanner itemCount={fridgeItemCount} />

      {/* Empty fridge warning (conditional) */}
      {fridgeItemCount === 0 && <EmptyFridgeWarning onAddProducts={handleAddProducts} />}

      {/* Source selection grid */}
      <SourceSelectionGrid userRecipeCount={userRecipeCount} onSourceSelect={onSourceSelect} />
    </div>
  );
}
