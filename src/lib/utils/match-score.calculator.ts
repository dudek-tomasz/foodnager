...
import type { RecipeIngredientDTO, FridgeItemDTO, AvailableIngredientDTO } from "@/types";
...
export type FridgeItemMap = Record<number, FridgeItemDTO>;
...
export class MatchScoreCalculator {
   ...
   */
  calculate(recipeIngredients: RecipeIngredientDTO[], availableFridgeItems: FridgeItemDTO[]): MatchScoreResult {
    ...
    const score = totalIngredients > 0 ? (fullyAvailableCount + partiallyAvailableCount * 0.5) / totalIngredients : 0;
   ...
   */
  static isGoodMatch(result: MatchScoreResult, threshold = 0.7): boolean {
