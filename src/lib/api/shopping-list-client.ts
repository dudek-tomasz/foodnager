...
import type { GenerateShoppingListDTO, ShoppingListResponseDTO } from "../../types";
 ...
 */
export async function generateShoppingList(data: GenerateShoppingListDTO): Promise<ShoppingListResponseDTO> {
  return apiClient.post<ShoppingListResponseDTO>("/api/shopping-list/generate", data);
}
 ...
 * @returns Lista brakujących składników
 */
export async function generateShoppingListForRecipe(recipeId: number): Promise<ShoppingListResponseDTO> {
