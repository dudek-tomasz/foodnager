...
export const AuthErrors = {
   ...
   */
  invalidCredentials: (message = "Nieprawidłowy email lub hasło") => new AuthError(message, "INVALID_CREDENTIALS", 401),
   ...
   */
  registrationFailed: (message = "Rejestracja nie powiodła się") => new AuthError(message, "REGISTRATION_FAILED", 400),

  /**
   * 400 - Logout failed
   */
  logoutFailed: (message = "Wylogowanie nie powiodło się") => new AuthError(message, "LOGOUT_FAILED", 400),
   ...
   */
  invalidToken: (message = "Token jest nieprawidłowy lub wygasł") => new AuthError(message, "INVALID_TOKEN", 400),
   ...
   * 401 - Session expired
   */
  sessionExpired: (message = "Sesja wygasła. Zaloguj się ponownie.") => new AuthError(message, "SESSION_EXPIRED", 401),

  /**
   * 401 - Unauthorized access
   */
  unauthorized: (message = "Brak dostępu. Zaloguj się.") => new AuthError(message, "UNAUTHORIZED", 401),
};
...
export function mapSupabaseAuthError(error: { message: string; status?: number }): AuthError {
  ...
  if (message.includes("invalid token") || message.includes("token expired") || message.includes("otp expired")) {
  ...
  return new AuthError("Wystąpił błąd podczas uwierzytelniania", "UNAUTHORIZED", error.status || 400);
}
