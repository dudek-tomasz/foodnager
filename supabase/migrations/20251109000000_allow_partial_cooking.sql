-- Migration: Allow cooking with partial ingredients
-- This migration modifies record_cooking_event to:
-- 1. Not fail when ingredients are missing
-- 2. Deduct only what's available in the fridge
-- 3. Track which ingredients were available vs missing
...
BEGIN
  ...
  LOOP
    ...
    -- Calculate how much we can actually deduct (min of required and available)
    v_qty_to_deduct := LEAST(v_available_qty, v_ingredient.required_qty);
    
    -- Only process if there's something to deduct
    ...
    -- Note: new_quantity will be v_available_qty - v_qty_to_deduct
    ...
    v_updated_items := v_updated_items || JSONB_BUILD_ARRAY(
      JSONB_BUILD_OBJECT(
        ...
        'required_quantity', v_ingredient.required_qty,
        'deducted_quantity', v_qty_to_deduct,
        'unit', v_ingredient.unit,
        'was_sufficient', v_available_qty >= v_ingredient.required_qty
      )
    );
  END LOOP;
...
'Records a cooking event and automatically updates fridge quantities using FIFO strategy. Allows cooking with partial ingredients - deducts only what is available. All operations are atomic.';
